...
CREATE OR REPLACE VIEW readmissions.readmissions AS
SELECT
    facility_id,
    facility_name,
    state,
...
FROM readmissions.readmissions GROUP BY condition ORDER BY pct_worse DESC;
...
SELECT state,
       ...
       ROUND(AVG(err), 4)                                    AS avg_err,
       ROUND(100.0 * SUM(worse_than_expected) / COUNT(err), 1) AS pct_worse
FROM readmissions.readmissions GROUP BY state HAVING COUNT(err) >= 30 ORDER BY avg_err DESC;

-- Q3. Worst Heart-Failure outliers (>= 300 discharges).
SELECT facility_name, state, discharges, err, readmissions
FROM readmissions.readmissions
...
FROM readmissions.readmissions WHERE err IS NOT NULL;
...
FROM readmissions.readmissions GROUP BY condition ORDER BY pct_suppressed DESC;
